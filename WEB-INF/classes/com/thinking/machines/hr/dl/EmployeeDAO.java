...
statement=connection.createStatement();
ResultSet resultSet;
resultSet=statement.executeQuery("select employee.employee_id,employee.name,employee.designation_code,employee.date_of_birth,employee.basic_salary,employee.gender,employee.is_indian,employee.pan_number,employee.aadhar_card_number,designation.title from employee inner join designation on employee.designation_code=designation.code order by employee.name");
int code;
String name;
int designationCode;
Date dateOfBirth;
...
gender=resultSet.getString(6);//ek char(1) hai
isIndian=resultSet.getBoolean(7);
panNumber=resultSet.getString(8).trim();
aadharCardNumber=resultSet.getString(9).trim();
title=resultSet.getString(10).trim();
...
if(gender.equals("M"))
{
employee.setGender(EmployeeDTO.MALE);
}
else
{
employee.setGender(EmployeeDTO.FEMALE);
}
...
if(gender.equals("M"))employeeDTO.setGender(EmployeeDTO.MALE);
